        ...
        <div class="card-body pt-3">
          ...
          <div class="tab-content pt-2">
            ...
            <div class="tab-pane fade show active profile-overview" id="profile-overview">
                        ...
                        $dbname = "velolib";
                        $conn = mysqli_connect($servername, $username, $password, $dbname);
                        if ($statement = mysqli_prepare($conn, "SELECT  * FROM station")) {
                            mysqli_stmt_execute($statement);
                            $result = mysqli_stmt_get_result($statement);
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr>";
                                    echo "<form method='POST' action='stationedit/updatestation.php  style='display:inline-table;'>";
                                    foreach ($row as $key => $value) {
                                        echo "<td><input type='text' name='$key' value='$value'></td>";
                                    }
                                    echo "<td>";
                               
                                    echo "<input type='hidden' name='id' value='" . $row["IDSTATION"] . "'>";
                                    echo "<input type='submit' name='submit' value='Update'>";
                                    echo "</form>";
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<form method='POST' action='stationedit/deletestation.php'>";
                                    echo "<input type='hidden' name='id' value='" . $row["IDSTATION"] . "'>";
                                    echo "<input type='submit' name='submit' value='delete'style='background-color:rgba(223, 22, 22, 0.925); color: white;'name='submit' value='delete'>";
                                    echo "</form>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                        ...
                        ?> 
                 <button id="add-bike-button">Ajouter une nouvelle station</button>

<!-- Formulaire pour ajouter un nouveau vélo (initialement caché) -->
<div id="add-bike-form" style="display:none;">

<form method="POST" action="stationedit/ajouterstation.php">
    <td>
        <tr><input type="text" name="modele" placeholder="adresse station"></tr>
